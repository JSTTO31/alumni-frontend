    ...
    <v-card class="rounded-lg py-3 px-4" :disabled="loading">
        <v-form @submit.prevent="save">
            ...
            <v-card-actions class="px-6">
                <v-btn class="text-capitalize px-6 text-error" prepend-icon="mdi-trash-can" variant="text" @click="$router.push({name: 'alumni-index-delete-works', query: {id: $route.query.id}})">Delete</v-btn>
                <v-spacer></v-spacer>
...
onMounted(() => {
    //@ts-ignore
    const work = user.value?.works.find(item => item.id == parseInt(route.query.id.toString()))
    if(!work){
