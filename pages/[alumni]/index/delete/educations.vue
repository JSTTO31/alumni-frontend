<template>
    <v-card :disabled="loading" class="px-4 py-3 rounded-lg">
      ...
      <v-card-actions>
           <v-spacer></v-spacer>
           <v-btn class="" variant="flat" @click="$router.push({name: 'alumni-index-edit-educations', query: {id: $route.query.id}})">Cancel</v-btn>
...
onMounted(() => {
   ...
   if(!findEducation) return router.push({name: 'alumni'})
