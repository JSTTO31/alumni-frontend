<template>
    <v-card class=" pb-0 rounded-lg border" flat>
        <div class="pa-5">
            <h4 class="font-weight-medium">Connection Request ({{ options.count }})</h4>
        ...
        <v-btn class="text-capitalize" block flat append-icon="mdi-arrow-right" @click="$router.push({name: 'network-connection-requests'})">Show more</v-btn>
    </v-card>
</template>
...
if(request_connections.value.length < 1){
    await execute()
}
</script>
