...
<template>
  <v-card v-ripple="false" class="rounded-lg pa-0 pb-4 border" flat :id="'card' + post.id" v-if="post">
    <div class="pa-4 pb-0 d-flex align-center">
      ...
      <v-spacer></v-spacer>
      <div class="mt-n5">
        <v-btn icon="mdi-bookmark" flat class="mr-1"></v-btn>
    ...
    <div class="px-5 pt-2">
      ...
      <div class="mb-5 d-flex">
        <v-btn flat class="text-capitalize mr-3" variant="text" rounded color="#1f6e8c" prepend-icon="mdi-heart"
          @click.stop="$post.post_reaction(post.id)" v-if="post.reacted">
          Heart
          <v-chip size="small" class="ml-5" v-if="post.reactions_count > 0">{{ post.reactions_count }} </v-chip>
        </v-btn>
        <v-btn flat class="text-capitalize mr-3" variant="text" rounded prepend-icon="mdi-heart-outline"
          @click.stop="$post.post_reaction(post.id)" v-else>
          Heart <v-chip size="small" class="ml-5" v-if="post.reactions_count > 0">{{ post.reactions_count }} </v-chip>
        </v-btn>
        <v-btn flat color="grey-darken-3" variant="text" rounded class="text-capitalize mr-3"
          prepend-icon="mdi-message-text-outline" @click.stop="focus">Comment <v-chip size="small" class="ml-5"
            v-if="post.comments_count > 0">{{ post.comments_count }} </v-chip>
        </v-btn>
        <v-btn flat color="grey-darken-3" variant="text" rounded class="text-capitalize mr-3"
          prepend-icon="mdi-share-outline" @click.stop="focus">Share <v-chip size="small" class="ml-5"
            v-if="post.comments_count > 0">{{ post.comments_count }} </v-chip>
        </v-btn>
      </div>
    </div>
    ...
    <v-card-actions class="d-flex flex-column">
      ...
      <div class="w-100 d-flex px-5 mt-3">
        <v-text-field label="Write your comment" single-line v-model="comment" variant="outlined"
          flat ref="text_field" @keyup.enter="commenthandler" rounded hide-details
          append-inner-icon="mdi-send" density="comfortable">
          </v-text-field>
      </div>
    </v-card-actions>
  </v-card>
