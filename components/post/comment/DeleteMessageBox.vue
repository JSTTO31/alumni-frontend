        ...
        <v-card-text class="pt-2">
            ...
            <v-card class="rounded-lg border" flat>
                <div class="pa-4 pb-0 d-flex align-center">
                    <avatar size="50" class="border mr-4" :user="comment.user">
                    </avatar>
                    <div class="underline">
                        <h5>{{ comment.user.name }}</h5>
                        <h6 @click.stop="" class="font-weight-regular text-caption font-weight-bold"
                            v-if="comment.user.general_information">{{
                                comment.user.general_information.department_name }}
                        </h6>
                    </div>
                </div>
                <v-card-text>
                    <h3 class="mb-2 font-weight-regular">{{ comment.text }}</h3>
                </v-card-text>
            </v-card>
        </v-card-text>
...
const {pending, status, execute} = await $comment.destroy(props.comment.id)
