     ...
     <div class="pa-0 d-flex mt-2 w-100" >
        ...
        <v-card flat class="w-100">
            <v-card-title class=" pt-0 pr-0 d-flex text-subtitle-1 d-flex font-weight-bold w-100 align-center">
                ...
                <v-spacer></v-spacer>
                <v-btn style="position: absolute;right: 0" icon="mdi-pencil" flat v-if="user && user.id == work.user_id" @click="$router.push({name: 'alumni-index-edit-works', query: {id: work.id}})"></v-btn>
            </v-card-title>
        </v-card>
    </div>
</template>
